...
function generatePDF(manualText, brand, codeRefPercentage) {
    return new Promise((resolve, reject) => {
          ...
          doc.on('error', reject);
                           ...
                           // --- Header ---
                           doc.fontSize(20).font('Helvetica-Bold').text('Matrix Manual', { align: 'center' });
          ...
          doc.fontSize(8).fillColor('#888888').text(
                  `Generated by Matrix Manual | ${new Date().toLocaleDateString()}`,
            { align: 'center' }
                );
          ...
          return res.status(400).send('Webhook signature verification failed');
           ...
           switch (event.type) {
             ...
             case 'customer.subscription.created':
                     console.log('Matrix Manual - Subscription created:', event.data.object);
                     break;
             case 'customer.subscription.deleted':
                     console.log('Matrix Manual - Subscription cancelled:', event.data.object);
                     break;
